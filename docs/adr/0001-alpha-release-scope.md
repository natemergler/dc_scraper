...
Mayor's Order, and D.C. Law locators already present in source-derived citations. Review workflow
contents, draft decisions, diagnostics, broader legal-source modeling, product demos, and civlab.org
integration remain outside the alpha release scope so the checkpoint can finish without pretending
to be a complete civic or legal database. The release may still publish review posture counts and
deferred-group summaries as readiness metadata; those summaries describe the state of review, not
additional civic facts. Alpha legal authority entries use one `dc.legal_authority` kind with
...
reconstruct common public-facing civic slices from raw relation rows. The authority affiliation view
is still emitted when the live DCGIS authority source is collected-empty; in that case the release
must explain the zero-row authority output through `source_coverage.csv`, `manifest.json`, and the
release README instead of fabricating authority entries.
