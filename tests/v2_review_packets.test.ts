...
Deno.test("review packet source lookup does not scale queries per item", async () => {
  ...
  assert(
    allItemsPrepareCount <= oneItemPrepareCount + 1,
    `packet source lookup should stay bounded; one item prepared ${oneItemPrepareCount}, all items prepared ${allItemsPrepareCount}`,
  );
});
